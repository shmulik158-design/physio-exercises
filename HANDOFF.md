...
- **מגבלה ידועה — הדפסה ממחשב אחר**: כל קובץ יורד לתיקיית ההורדות **של המחשב שממנו הודפס**, לא מסונכרן אוטומטית לשום מקום. אין API בדפדפן שמאפשר לדף סטטי לצרף/לעדכן קובץ קיים אחד לאורך זמן ובין דפדפנים באופן אמין (File System Access API קיים בכרום בלבד, דורש הרשאה חוזרת ושברירית) — זו הסיבה שנבחר "קובץ נפרד לכל הדפסה" ולא "ledger" יחיד, ראו דיון מפורט בשיחה. **המלצה מעשית (הגדרת דפדפן, לא קוד)**: להגדיר את תיקיית ההורדות של הדפדפן (בכל מחשב שממנו מדפיסים) לתיקייה מסונכרנת ב-OneDrive — אותה גישה שכבר קיימת לגיבוי תמונות המקור. אם כמה מחשבים מצביעים לאותה תיקיית OneDrive, כל ההדפסות מתאספות במקום אחד בפועל, בלי קוד נוסף. שם הקובץ כולל את שם המטפל (מ-2026-08-30) בדיוק כדי שקבצים ממחשבים שונים יסתדרו יפה אם וכשיתמזגו לתיקייה אחת.
