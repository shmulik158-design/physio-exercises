...
8. **Content drift ללא גרסתיות** (זוהה ע"י 3 מתוך 5 יועצי המועצה, ללא תיאום): עריכת תרגיל קיים משנה אותו רטרואקטיבית לכולם, בלי "עודכן לאחרונה" על התרגיל ובלי snapshot של מה שהודפס בפועל בכל רגע. לא טופל עדיין — שווה תשומת לב אם ריבוי מטפלים יתרחב.
9. **LICENSE file**: המועצה המליצה להוסיף (ריפו ציבורי בלי רישיון = "כל הזכויות שמורות" כברירת מחדל, לא "פתוח לכולם"). טרם נוסף — החלטה משפטית שהבעלים צריך לקבל (MIT / All Rights Reserved / אחר), לא Claude.
