...
- ✅ **סיסמה על כלי הניהול + לוג הדפסה append-only** (2026-08-30) — ראו "אבטחה ו-audit trail" למטה. ⚠️ **דורש הפעלה מחדש של `add-exercise.bat` — בפעם הראשונה אחרי ההפעלה מחדש תופיע מסך "הגדרת סיסמה" במקום הכלי הרגיל, זה צפוי ולא תקלה.**
...
**Print log — append-only, ב-`index.html`** — כל לחיצה על "הפק דף להדפסה" מורידה גם קובץ JSON קטן (`print-log_<timestamp>.json`, לתיקיית ההורדות של הדפדפן) עם: מטפל, שפה, ולכל תרגיל — הטקסט/מינון **כפי שהודפסו בפועל** באותו רגע (לא מצביע חי ל-`exercises.json` שעלול להשתנות אחר כך). קובץ נפרד לכל הדפסה — אין קובץ מרכזי שנכתב-מעל, ולכן אין מה "לערוך" בדיעבד מבנה.
- **`index.html` הוא static, בלי backend** — לכן הלוג ממומש כהורדת קובץ צד-לקוח (Blob + `<a download>`), לא כתיבה לתיקייה מקומית בשרת. זה עובד זהה בין GitHub Pages הציבורי לעותק מקומי.
- **שם המטופל לא נכלל בלוג, במכוון.** לוג append-only שלא ניתן לערוך/למחוק מתנגש עם זכות מטופל לתקן/למחוק מידע אישי (ראו chairman verdict בdoc של council session), וגם הופך את ההחלטה הקיימת "שם מטופל לא נשמר בשום מקום" — לא סתם תוספת, זה שינוי עקרון אם היה נכלל. הלוג מוכיח **מה** נאמר, לא **למי** — הקישור למטופל ספציפי הוא תפקיד הדף המודפס/התיק הקליני, לא של הכלי הזה.
- **מגבלה ידועה — הדפסה ממחשב אחר**: כל קובץ יורד לתיקיית ההורדות **של המחשב שממנו הודפס**, לא מסונכרן אוטומטית לשום מקום. אין API בדפדפן שמאפשר לדף סטטי לצרף/לעדכן קובץ קיים אחד לאורך זמן ובין דפדפנים באופן אמין (File System Access API קיים בכרום בלבד, דורש הרשאה חוזרת ושברירית) — זו הסיבה שנבחר "קובץ נפרד לכל הדפסה" ולא "ledger" יחיד. שם הקובץ כולל את שם המטפל (מ-2026-08-30) כדי שקבצים ממחשבים שונים יסתדרו יפה אם וכשיתמזגו לתיקייה אחת.

### למה "לוגי ההדפסה של עמיתים לא מגיעים אליי" **אינה** בעיה שצריך לפתור — חשוב (2026-08-30)

בעקבות דיון עם המשתמש אחרי בניית ה-print log: המשתמש כבר משתף את הקישור הציבורי עם כמה עמיתים, והעלה שאלה טבעית — אם עמית מדפיס, ה-JSON נוחת על **המחשב שלו**, לא של המשתמש. נבדקו ונדחו כמה פתרונות (תיקיית OneDrive משותפת — העמיתים לא ישתפו פעולה עם שינוי הגדרת דפדפן; שירות email צד-שלישי כמו EmailJS — תלות חיצונית ראשונה בפרויקט, נתונים עוברים דרך גורם שלישי, quota — לא נבנה).

**התובנה שסגרה את זה, מהמשתמש עצמו**: דף התרגילים המודפס **אינו** ואף פעם לא היה אמור להיות חלק מהרשומה הרפואית. הוא כלי הנגשה למטופל (המחשה ויזואלית של תרגיל שהמטפל כבר החליט עליו), לא תיעוד קליני. **התיעוד הקליני האמיתי — מה בדיוק נקבע למטופל, מתי — קורה במערכת הנפרדת של קופת החולים**, בלי שום קשר לכלי הזה. זה משנה לגמרי את חישוב הסיכון: אם יש אי-בהירות מה בדיוק "המדפסת אמרה" למטופל בזמן נתון, זה כבר לא שאלה שהכלי הזה צריך לפתור — כי הכלי הזה מעולם לא היה הרשומה הסמכותית.

**המסקנה**: לא נבנה שום מנגנון מרכוז/שיתוף לוגי הדפסה בין משתמשים. `print-log` נשאר כמו שהוא — עותק אישי צנוע למי שמדפיס, בלי יומרה להיות משהו יותר מזה. **במקום זאת נוסף disclaimer קבוע במסך הבילדר** (לא בדף המודפס — זה מיועד למטפל שמשתמש בכלי, לא למטופל): "השימוש בפלטפורמה... באחריות המטפל/ת המשתמש/ת בלבד... הכלי אינו מהווה תחליף לתיעוד קליני." זו ה"תשובה" האמיתית — לא תשתית, אלא הבהרה מפורשת של גבול האחריות שממילא היה נכון מההתחלה.

**אם זה עולה שוב בעתיד**: אל תתחיל מ"איך נרכז לוגים" — תתחיל מהשאלה הזו: האם דף התרגילים המודפס אמור להיות ראיה לתיעוד קליני? התשובה כבר נקבעה: לא.
